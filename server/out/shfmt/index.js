...
class Formatter {
  ...
  async format(document, formatOptions, shfmtConfig) {
    if (!this._canFormat) {
      return []
    }
    return this.executeFormat(document, formatOptions, shfmtConfig)
  }
  async executeFormat(document, formatOptions, shfmtConfig) {
  ...
  async getShfmtArguments(documentUri, formatOptions, lspShfmtConfig) {
    ...
    if (filepathMatch) {
      ...
      if (!lspShfmtConfig?.ignoreEditorconfig) {
        ...
        logger_1.logger.debug(
          `Shfmt: found .editorconfig properties: ${JSON.stringify(editorconfigProperties)}`,
        )
    ...
    const indentation = formatOptions?.insertSpaces ? formatOptions.tabSize : 0
  ...
  async runShfmt(document, formatOptions, shfmtConfig) {
