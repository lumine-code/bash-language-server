  ...
  if (node.type === 'command') {
    ...
    if (
      ...
      if (previousCommentNode?.text.includes('shellcheck')) {
        const directives = (0, directive_1.parseShellCheckDirective)(
          previousCommentNode.text,
        )
        const sourcedPath = directives.find(
          (0, discriminate_1.discriminate)('type', 'source'),
        )?.path
        ...
        const rootFolder = directives.find(
          (0, discriminate_1.discriminate)('type', 'source-path'),
        )?.path
...
function toFilePath(pathOrUri) {
  return pathOrUri.startsWith('file:')
    ? (0, node_url_1.fileURLToPath)(pathOrUri)
    : pathOrUri
}
